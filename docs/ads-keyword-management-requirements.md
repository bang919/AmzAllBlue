...
- “调整 AI 策略规则”按 Profile 只保存当前规则，包含全局说明、普通/主推/已稳定的打法，以及最高竞价、单次调整幅度、最高预算、最高位置加价和最短观察期等硬限制；不保留无业务价值的历史规则版本。每日批量分析下可选择请求批准、替我审批或完全批准；默认请求批准。
- 每个关键词可以保存独立的“调整目标”。每次分析使用最新策略版本和当前关键词目标，但分析表永久保留当时的完整输入快照。
- AI 输入默认包含最近 30 天关键词综合与各广告位置的每日指标、自然/广告排名、当前 Campaign 和投放单元设置、当前 FBA 库存与近 30 天日均销量、SIF 建议竞价，以及最近 30 天 AI 建议与执行历史；分析窗口可在策略规则中调整。
...
- AI 不允许自动执行真实广告操作，所有建议始终必须人工确认。
- 手动分析在服务端后台运行；切换关键词或刷新页面后会从分析运行记录恢复“分析中/完成/失败”状态。每日批量分析可在策略规则中启用并设置时间，使用一次 CCAI 批量请求分析所有已填写调整目标的关键词。
