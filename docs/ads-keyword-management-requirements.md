# 广告关键词管理需求

## 目标

在 AmzAllBlue 项目中新增一套广告关键词管理系统。

这套系统按照新的规则创建和管理广告，不整理、不导入，也不接管 Amazon 账户中原有的其他广告。

系统只管理以下广告组合中的广告：

```text
AmzAllBlue_ERP
```

不在 `AmzAllBlue_ERP` 广告组合中的广告，系统不读取、不修改、不暂停。

## 核心使用方式

前端使用表格管理关键词：

- 一个产品可以添加多个关键词。
- 一个关键词在主表中占一行。
- 关键词分为 `普通`、`主推`、`已稳定` 三组。
- 每个关键词可以分别开启或暂停 `精准`、`词组`、`广泛`。
- 点击关键词行后，展开查看每种投放方式的详细数据。

主表大致显示：

```text
父 ASIN | 子 ASIN | 关键词 | 分组 | 精准 | 词组 | 广泛 | 花费 | 销售额 | ACOS
```

展开后显示：

```text
广告类型 | 匹配方式 | 状态 | 出价 | 顶部加价 | 曝光 | 点击 | 花费 | 订单 | 销售额 | ACOS
```

## 广告创建位置

系统新建的广告统一创建在：

```text
广告 Profile：用户当前选择的 Profile，默认 US
广告组合：AmzAllBlue_ERP
```

第一版只创建和管理 SP 关键词广告。

每个“关键词 × 子 ASIN × 匹配方式”创建一套独立 Campaign：

```text
SP 精准：EXACT
SP 词组：PHRASE
SP 广泛：BROAD
```

三套投放相互独立，可以使用不同的出价、顶部加价和启停状态。

只有用户开启某种匹配方式时才创建对应广告。关闭时暂停广告，不删除，重新开启时继续使用原广告。

### 默认创建模板

首次创建使用以下默认设置：

```text
Campaign 日预算：8（使用当前 Profile 的货币单位，US 即 8 USD）
Campaign 竞价策略：固定竞价
默认勾选的匹配方式：EXACT
PHRASE 和 BROAD：默认不勾选，勾选后才创建
默认出价：0.2（使用当前 Profile 的货币单位）
顶部搜索结果加价：200%
开始日期：立即
初始状态：ENABLED
结束日期：不设置
```

顶部加价会叠加在基础出价上。例如基础出价为 `0.2`、顶部加价为 `200%` 时，顶部搜索结果位置的竞价最高可达到 `0.6`。

创建页面可以读取 Amazon Ads API 或 SIF 返回的建议竞价，作为推荐值展示或填入表单，用户确认后再创建。

系统按 Profile 分别保存上一次创建广告时使用的设置。下一次创建时自动套用为模板，所有值仍可修改；用户修改并成功创建后，更新该 Profile 的模板。

### 已确认的安全与对象处理规则

- 如果当前 Profile 中不存在 `AmzAllBlue_ERP`，页面只显示缺失，不自动创建。用户查看 Portfolio 名称、Profile 和创建参数并明确确认后，系统才允许创建。
- 广告模块只列出已经设置父 ASIN 内部名的产品组；未设置内部名的父 ASIN 不显示。
- 广告模块中的父 ASIN 标签支持拖拽排序，顺序保存到本地数据库，刷新页面后保持不变。
- 创建表单允许多选子 ASIN；系统将其拆成多个“关键词 × 子 ASIN”的独立投放对象。每种已选匹配方式为每个子 ASIN 创建独立 Campaign、一个 Ad Group、Product Ad 和 Keyword Target。同一个父 ASIN 下，相同关键词不得重复投放到同一子 ASIN；选择其他子 ASIN 可以继续添加。
- 一个子 ASIN 对应多个 Seller SKU 时，沿用 FBA 库存页面的规则：先选择最新实时总货物数最高的 SKU，并列时再按默认最近 30 天的日销量选择；仍并列时保留多个选项，但同一子 ASIN 最终只能选择一个 Seller SKU。创建表单用商品卡片展示图片、子 ASIN 内部名、ASIN 和 Seller SKU，不展示库存数与销量，也不会在未展示 SKU 的情况下自动提交。
- 用户从关键词中移除子 ASIN 时，系统暂停对应的 Ad Group、Product Ad 和 Keyword Target，不删除 Amazon 对象，也不删除历史表现。
- 如果 `AmzAllBlue_ERP` 中存在数据库未记录的人工广告对象，系统不导入、不接管，并显示冲突错误，提示用户先在 Amazon 后台删除人工广告。
- 修改关键词分组后，本地保存目标分组。Campaign 或 Ad Group 部分改名失败时显示 `同步未完成`、失败对象和原因，重试时只继续尚未成功的对象。
- 创建任何真实 Portfolio 或广告前，必须先展示 Profile、Portfolio、Campaign、Ad Group、子 ASIN、Seller SKU、Product Ad、Keyword Target、预算、出价、顶部加价、日期和状态等完整参数，并由用户明确确认。
- 创建计划尚未产生任何 Amazon ID 时，用户可以“关闭创建计划”。系统删除该计划的本地关键词草稿、Campaign、Ad Group 投放单元、操作预览和步骤记录；如果已经创建了任一 Amazon 对象，则禁止删除计划，只允许暂停或从失败步骤续传。
- 用户确认创建后，界面必须持续展示操作的真实状态：等待确认、正在创建、创建成功或创建失败。网络中断时通过操作状态接口核对数据库记录，不得无提示恢复为初始确认按钮；失败时展示错误并允许从未完成步骤重试。创建成功后自动关闭确认弹窗并刷新关键词详情。

## 广告结构

第一版采用以下结构：

```text
Portfolio: AmzAllBlue_ERP
  Campaign: 一个关键词、一个子 ASIN的一种匹配方式
    Ad Group: 该子 ASIN 的一个广告组
      Product Ad: 只投放该广告组对应的子 ASIN
      Keyword Target: 对应关键词和匹配方式
```

例如关键词 `wall organizer` 同时开启三种匹配方式，会为每个子 ASIN 创建三个 Campaign；投放两个子 ASIN 时共创建六个 Campaign：

```text
wall organizer / EXACT
wall organizer / PHRASE
wall organizer / BROAD
```

这样可分别控制每个子 ASIN、每种匹配方式的预算、位置加价和出价。

每个 Campaign 只对应一个子 ASIN，因此预算、竞价策略和位置加价均可精准控制。需注意：多个子 ASIN 会相应增加日预算上限。

## 命名规则

### Campaign 名称

格式：

```text
ERP__产品名__PARENT-父ASIN__ASIN-子ASIN__关键词__分组__SP__匹配方式__TS-创建批次
```

示例：

```text
ERP__WallBag__PARENT-B0B723ZTBD__wall organizer__普通__SP__EXACT__TS-20260713T093015123Z
ERP__WallBag__PARENT-B0B723ZTBD__wall organizer__主推__SP__PHRASE__TS-20260713T093015123Z
ERP__WallBag__PARENT-B0B723ZTBD__wall organizer__已稳定__SP__BROAD__TS-20260713T093015123Z
```

字段含义：

| 字段 | 示例 | 说明 |
| --- | --- | --- |
| 系统标识 | `ERP` | 表示由本系统管理 |
| 产品名 | `WallBag` | 产品的系统简称 |
| 父 ASIN | `B0B723ZTBD` | 广告所属父产品 |
| 关键词 | `wall organizer` | 实际投放关键词 |
| 分组 | `普通` | `普通`、`主推`、`已稳定` |
| 广告类型 | `SP` | 第一版固定为 SP |
| 匹配方式 | `EXACT` | `EXACT`、`PHRASE`、`BROAD` |
| 创建批次 | `20260713T093015123Z` | 创建计划时生成一次，预览、创建和失败续传保持不变 |

字段之间统一使用双下划线 `__` 分隔。

### Ad Group 名称

Ad Group 使用 Campaign 名称，并在末尾增加投放的子 ASIN：

```text
ERP__WallBag__PARENT-B0B723ZTBD__wall organizer__主推__SP__EXACT__TS-20260713T093015123Z__ASIN-B0B37GZ7MM
```

一个 Ad Group 只投放一个子 ASIN，因此 Ad Group 名称必须包含该子 ASIN。

### 分组改名

用户在前端将关键词从 `普通` 调整到 `主推` 时，系统同步修改对应 Campaign 和它下面所有 Ad Group 的名称。

修改前：

```text
ERP__WallBag__PARENT-B0B723ZTBD__wall organizer__普通__SP__EXACT__TS-20260713T093015123Z
```

修改后：

```text
ERP__WallBag__PARENT-B0B723ZTBD__wall organizer__主推__SP__EXACT__TS-20260713T093015123Z
```

只修改名称中的分组，不重新创建广告，不丢失历史数据。

### 名称长度和特殊字符

- 产品名使用项目中维护的产品简称。
- 关键词保留原文，方便在 Amazon 后台识别。
- 如果名称超过 Amazon 限制，系统自动缩短产品名或关键词显示部分。
- 系统在数据库保存 Amazon 返回的 Campaign ID、Ad Group ID 和 Target ID，名称主要用于查看和管理，不作为唯一识别依据。
- `creation_batch` 同时保存到关键词、Campaign 和 Ad Group 投放单元。Campaign 使用 `父 ASIN + 子 ASIN + creation_batch + 广告类型 + 匹配方式` 形成稳定本地对象键；归档旧批次后重新创建会生成新的时间戳，名称和对象键都不会冲突。

## 分组规则

关键词有三个分组：

```text
普通
主推
已稳定
```

分组只表示运营管理状态，不自动决定广告是否开启。

用户可以随时修改分组，系统通过改名把新的分组同步到 Amazon Ads。

### 普通

目标：测试关键词价值，并通过较低成本获取有利润的订单。

策略：

- 使用相对保守的出价积累数据。
- 可以使用词组和广泛匹配发现有效搜索词。
- 使用精准匹配验证关键词本身的转化能力。
- 有稳定转化并值得提升自然排名的词，可以转为 `主推`。
- 流量不大但能够稳定盈利的长尾词，可以继续留在 `普通`。
- 长期只有点击而没有订单的词，降低出价或暂停。

重点指标：

```text
曝光
点击
CTR
CPC
订单
转化率
ACOS
广告利润
```

### 主推

目标：通过增加关键词广告成交，提升关键词自然排名，最终增加自然流量和自然订单。

策略：

- 以精准匹配作为主要投放方式。
- 根据表现提高精准出价和顶部搜索结果加价。
- 在可控范围内允许比利润型广告更高的 ACOS。
- 观察广告订单增长是否带来自然排名、自然订单和总订单增长。
- 同时关注 FBA 库存，避免排名提升后因断货失去排名。
- 自然排名和订单连续一段时间稳定后，转为 `已稳定`。

重点指标：

```text
关键词自然排名
广告订单
总订单
自然订单
自然订单占比
顶部曝光
ACOS
TACOS
库存可售天数
```

### 已稳定

目标：保持已有自然排名和销量，同时减少不必要的广告花费，提高整体利润。

策略：

- 保留必要的精准广告，维持关键词曝光和成交。
- 逐步测试降低出价或顶部搜索结果加价。
- 减少词组和广泛匹配中的无效流量。
- 观察降低广告投入后，自然排名和自然订单是否仍然稳定。
- 如果自然排名或自然订单明显下降，将关键词重新转为 `主推`。

重点指标：

```text
关键词自然排名稳定性
自然订单
自然订单占比
ACOS
TACOS
广告利润
总销量
```

### 分组流转

典型流转过程：

```text
普通 → 主推 → 已稳定
             ↓
          排名下降
             ↓
            主推
```

三个分组的核心定位：

```text
普通：测试关键词和低价盈利
主推：提升关键词自然排名
已稳定：保持排名并提高利润
```

## 数据记录

广告表现按天保存，方便查询任意日期范围、比较趋势和后续进行 AI 分析。

关键词完成创建后，右侧详情区切换为历史数据面板，不再显示“预览并创建”。历史面板支持按子 ASIN 和匹配策略（全部、精准、词组、广泛）筛选，日期范围可调整；折线图横轴为日期，最多同时选择两个纵轴指标。第一版可选出价、顶部加价、实际 CPC、曝光、点击、花费、订单、销量和销售额。关键词自然位和广告位必须在接入独立排名数据源后启用，不得用广告报表数据推测或伪造。

前端的一行仍然代表一个父 ASIN 下的一个关键词；数据库不要把产品配置和每天的表现数据全部重复写在一张表里。

### 关键词

每个关键词保存：

```text
父 ASIN
关键词
分组
创建时间
更新时间
```

### 子 ASIN

同一关键词可投放多个子 ASIN，但每条“关键词投放对象”只对应一个子 ASIN；相同父 ASIN、关键词、子 ASIN 的活动投放对象唯一。

每个子 ASIN 使用独立 Ad Group，不直接保存为数组。关联记录包括：

```text
投放单元 ID
子 ASIN
Seller SKU
Amazon Ad Group ID
Amazon Product Ad ID
状态
出价
```

这样可以为不同子 ASIN 单独设置出价，也可以单独增加、暂停或移除某个子 ASIN。

### Campaign 与投放单元

每个子 ASIN、每种匹配方式保存一条 Campaign 记录：

```text
广告类型：SP / SB / SD
匹配方式：EXACT / PHRASE / BROAD
Campaign ID
状态：ENABLED / PAUSED
日预算
竞价策略
顶部加价
```

Campaign 下每个子 ASIN 保存一条 Ad Group 投放单元记录：

```text
Ad Group ID
Product Ad ID
Target ID
子 ASIN
状态：ENABLED / PAUSED
出价
```

第一版只使用 `SP` 和三种关键词匹配方式。

Campaign 代表一个关键词、一个子 ASIN的一种匹配方式；Campaign 下仅保留该子 ASIN 的一个 Ad Group 投放单元。

### 每日表现

竞价、日预算和各广告位置加价使用独立的每日设置快照。同一 Amazon 广告日期内多次修改时，以最后一次成功值覆盖当天快照；不同日期的快照不回写。曝光、点击、花费、订单、销售额和 CPC 仍来自 Amazon Ads 每日报表，不能把全天表现完全归因于当天最后一次设置。

每个实际存在的子 ASIN Ad Group 投放单元每天最多保存一条：

```text
日期
Ad Group 投放单元 ID
曝光
点击次数
花费
订单数
购买产品数
销售额
数据同步时间
```

同一天重复同步时更新原记录，不重复新增。

以下指标查询时计算，不重复保存：

```text
CPC = 花费 / 点击次数
CTR = 点击次数 / 曝光
ACOS = 花费 / 销售额
ROAS = 销售额 / 花费
转化率 = 订单数 / 点击次数
```

没有创建、已经暂停且当天没有表现数据的投放单元，不需要插入全为零的每日记录。

如果一个关键词投放多个子 ASIN，每个子 ASIN Ad Group 分别保存每日表现；主表再将这些明细汇总为该关键词的数据。

### 同步规则

- 所有日期按当前 Amazon Ads Profile 的时区计算。
- 系统每小时尝试同步一次今天的 Ad Group 日报表；一份报表覆盖当前 Profile 内全部受管 Ad Group，并由本地 ID 映射到子 ASIN 和关键词。同一同步任务尚未结束时，不重复启动。
- 每天另外同步并覆盖最近 30 天的 Ad Group 日报表，用于补齐延迟归因的订单和销售额。
- 展示位置（顶部搜索等）报表作为补充数据单独同步，不阻塞关键词历史图和核心表现数据入库。
- 同步通过 Amazon Ads 报表异步完成，遇到 API 限流时排队重试，不并发重复申请相同报表。
- 页面日期弹窗参考 FBA 库存模块：已经成功请求并保存的日期显示绿点。
- 最近两天的数据仍可能变化，即使已经请求，也不显示绿点。
- 用户可以手动选择日期范围重新同步，结果按日期和投放单元覆盖，不重复新增。

### 自然排名

关键词自然排名后续通过 SIF API 或 MCP 获取，并按日期保存。

第一版广告数据同步不依赖 SIF；SIF 暂时不可用时，不影响 Amazon Ads 广告的创建、启停和表现数据同步。

### 创建失败处理

创建 Campaign、Ad Group、Product Ad 和 Keyword Target 时，每成功一步立即保存 Amazon 返回的 ID。

如果中途失败：

- 将该投放单元标记为 `创建未完成`，并显示失败步骤和原因。
- 已成功创建的对象不自动删除。
- 用户重试时从失败步骤继续。
- 重试前先检查已经保存的 Amazon ID，不重复创建已成功的对象。

### 数据量

按每个关键词投放一个子 ASIN，以 `100 个产品 × 100 个词 × 3 种 SP 匹配方式` 的满负荷估算：

```text
每天约 30,000 条
十年约 1.095 亿条
预计数据库和索引约 25～50 GB
```

Windows 数据盘建议为数据库、备份和运行空间预留约 `150 GB`。

第一版保留每日数据。数据积累多年后，可以保留最近两年的每日数据，并把更早的数据汇总为月度数据。

如果同一个关键词投放多个子 ASIN，每增加一个子 ASIN，会相应增加该关键词的 Ad Group 和每日数据量。

## 第一版功能

### AI 建议与授权执行

- “调整 AI 策略规则”按 Profile 只保存当前规则，包含全局说明、普通/主推/已稳定的打法，以及最高竞价、单次调价金额上限、单次调价比例上限、最高预算、预算调整比例上限、最高位置加价、最短观察期和“带入近期 AI 记录（天）”等硬限制；不保留无业务价值的历史规则版本。每日批量分析下可选择需要用户批准、AI替用户审批或自动批准执行；默认需要用户批准。
- 每个关键词可以保存独立的“调整目标”。每次分析使用当前策略和关键词目标，分析表永久保留当时的完整输入快照。
- AI 输入默认包含最近 30 天关键词综合与各广告位置的每日指标、自然/广告排名、当前 Campaign 和投放单元设置、当前 FBA 库存与近 30 天日均销量、SIF 建议竞价，以及最近 7 天的 AI 分析和建议行动状态；分析窗口和带入天数可在策略规则中调整。为控制批量请求成本，近期 AI 历史只传每词最多 3 次压缩分析、每次最多 1 条建议；填 0 不传。
- 服务端同时计算最近 7 天、前 7 天和最近 30 天摘要及环比，减少模型自行计算造成的错误；原始每日数组仍作为证据提供。
- CCAI 输入和输出均使用标准 JSON。返回内容必须通过动作白名单、字段结构、对象归属、当前值和策略边界校验；失败只记录分析错误，不展示建议。
- 只有数据库中真实存在的 `PENDING` 建议才在关键词旁显示感叹号。新分析成功后，旧的待处理建议转为“已被新分析取代”。
- 用户可以逐条拒绝或确认建议。确认执行前，服务端再次检查当前竞价、预算、位置加价、状态或分组是否仍与建议的 `before` 一致；不一致时要求重新分析。
- 建议生成、被新分析取代、用户拒绝、用户确认、执行开始、执行成功和执行失败均写入数据库事件流水；执行成功后记录观察天数与复盘到期时间。
- 建议行动支持需要用户批准、AI替用户审批和自动批准执行三种模式；无论是否自动执行，都必须先通过服务端对象归属、当前值与安全边界校验，并完整记录执行结果。
- 手动分析在服务端后台运行；切换关键词或刷新页面后会从分析运行记录恢复“分析中/完成/失败”状态。每日批量分析可在策略规则中启用并设置北京时间；前端只触发一个后台父任务，服务端按每批最多 5 个关键词和约 9 万字符输入上限动态拆分 CCAI 请求，单批失败不影响其他批次。广告表现与日期数据仍按 Amazon Profile 时区采集和归属。
- 服务端先执行确定性预判。缺少有效广告对象，或最近 30 天完全没有广告流量且尚未达到最短观察期时，直接生成带摘要的 `REQUEST_MORE_DATA` 并标记为“规则预判”，不消耗 CCAI；零流量但已达到观察期时仍进入模型，让 AI 结合建议竞价、排名和投放资格判断是否应调整。
- `REQUEST_MORE_DATA` 仅提示需要补充或等待数据，不属于可执行建议，不显示关键词感叹号；用户可以点击“确认已知晓”，系统记录确认事件。
- 关键词详情仅展示最近一条建议行动；“查看分析历史”按每次分析分为调整目标与建议、AI 分析、建议行动三行，并可按需读取当时的完整输入 JSON、Campaign 设置、出价、库存和 30 天表现快照。

### 关键词详情交互

- 关键词标题与运营分组放在同一行；运营分组使用紧凑选择器。打开下拉菜单并悬停普通、主推或稳定时，显示对应的运营定义；此定义也作为后续 AI 分析建议的优先级语义，不直接触发广告操作。
- 普通：用于常规测试和稳定维护，AI 主要从效率、趋势和异常角度给出建议，不因该分组自动调价、启停或改变预算。
- 主推：用于当前需要更多流量和销售的重点关键词，AI 更关注展示份额、顶部搜索、排名、增量订单、可承受 ACOS 与预算消耗，建议按策略规则中的处理建议行动执行。
- 稳定：用于已验证转化与成本表现、暂不需要频繁调整的关键词，AI 重点观察 ACOS、销量与曝光异常，并提示是否需要复盘或恢复测试。
- 历史数据筛选器使用紧凑的子 ASIN、策略、指标一、指标二控件。
- 日期范围沿用 FBA 库存的单一日期弹窗交互，支持今天、昨天、最近 7 天、最近 30 天、最近 30 天（不含近 2 天）。
- 已完整同步广告表现的日期标记绿色，已选日期范围标记浅绿色；部分同步日期显示橙色状态。
- Campaign 卡片头部显示该子 ASIN 的图片、内部名、创建状态和启停操作；暂停、开始、停止只作用于该子 ASIN 的 Campaign。

第一版只实现：

1. 识别并使用 `AmzAllBlue_ERP` 广告组合。
2. 选择产品并新增关键词。
3. 设置关键词为 `普通`、`主推` 或 `已稳定`。
4. 创建 SP 精准、词组、广泛广告。
5. 默认只勾选 EXACT，并使用可编辑、可记忆的创建模板。
6. 一个 Campaign 只支持一个子 ASIN Ad Group；多个子 ASIN 自动拆成多个 Campaign。
7. 分别开启、暂停或停止每个子 ASIN、每种匹配方式的 Campaign；卡片作为对应投放对象的唯一启停入口。
8. Campaign 可设置日预算，以及顶部搜索、其余搜索结果、商品页面三类位置加价（0～900%）；新增关键词时其余搜索与商品页面默认 0%。
9. 子 ASIN 投放单元可设置默认出价，并同步更新其对应 Keyword Target 出价。
10. 修改关键词分组并同步 Campaign 和 Ad Group 名称。
11. 表格显示曝光、点击、花费、订单、销售额和 ACOS。
12. 点击已创建关键词查看可按子 ASIN、匹配策略和日期筛选的双指标历史图表，并可折叠查看投放对象与启停管理。
13. 每小时同步今天，每天滚动覆盖最近 30 天的广告表现。
14. 日期弹窗标记已请求数据，最近两天不显示完成绿点。
15. 创建中途失败后可以从失败步骤继续，不重复创建对象。
16. 配置版本化 AI 策略和关键词调整目标，通过 CCAI 生成结构化建议，并按处理建议行动模式请求确认或自动执行。
17. 保存每次 AI 分析的输入输出快照、建议状态、用户决定、执行结果和后续复盘时间。

第一版暂不实现：

- SB 和 SD 广告。
- 无人确认的 AI 自动调价。
- 自动管理 `AmzAllBlue_ERP` 之外的广告。
- 自动导入 Amazon 账户已有广告。

## 后续扩展

第一版稳定后再增加：

- 自动生成执行后 3/7/30 天效果复盘，并评估策略建议质量。
- SB、SD 等其他广告类型。
- 搜索词分析和否定关键词建议。
