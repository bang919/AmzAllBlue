...
- “调整 AI 策略规则”按 Profile 只保存当前规则，包含全局说明、普通/主推/已稳定的打法，以及最高竞价、单次调整幅度、最高预算、最高位置加价、最短观察期和“带入近期 AI 记录（天）”等硬限制；不保留无业务价值的历史规则版本。每日批量分析下可选择请求批准、替我审批或完全批准；默认请求批准。
...
- 建议行动支持请求批准、替我审批和完全批准三种模式；无论是否自动执行，都必须先通过服务端对象归属、当前值与安全边界校验，并完整记录执行结果。
...
16. 配置版本化 AI 策略和关键词调整目标，通过 CCAI 生成结构化建议并由用户逐条确认执行。
